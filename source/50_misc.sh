# Service detection
function has_ripgrep() {
  [[ $(command -v rg >/dev/null 2>&1) ]] || return 1
    }

function has_gls() {
  [[ $(command -v /usr/local/bin/gls >/dev/null 2>&1) ]] || return 1
    }
...
function has_tomcat() {
  [[ -e /opt/tomcat_latest/bin ]] || return 1
    }

function has_systemd() {
  [[ -e /etc/systemd ]] || return 1
    }
...
function has_app() {
  for app in docker nvm tomcat java clojure ripgrep gls gpg_agent ssh_agent systemd; do
    has_$app; [[ $? == ${1:-0} ]] && echo $app
  done
}
